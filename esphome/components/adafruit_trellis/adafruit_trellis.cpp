...
static const char *const TAG = "adafruit_trellis";
...
void JbdBms::proces_response_(void)
{
  ...
  for(;;) {
    ...
    while (this->available()) {
      uint8_t byte;
      this->read_byte(&byte);
      if (this->parse_adafruit_trellis_byte_(byte)) {
        this->last_byte_ = now;
      } else {
...
bool JbdBms::parse_adafruit_trellis_byte_(uint8_t byte) {
  ...
  this->on_adafruit_trellis_data_(function, data);

  // return false to reset buffer
  return false;
}

void JbdBms::on_adafruit_trellis_data_(const uint8_t &function, const std::vector<uint8_t> &data) {
